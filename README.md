...
### Для обновления используется следующая логика:

1. Проверяем, что начальный ID обновления (U) больше lastUpdateId.
2. Проверяем, что конечный ID обновления (u) больше lastUpdateId.

3. Удаляем из ордербука все заявки с объемом "0".

4. Добавляем новые заявки или обновляем существующие в ордербуке на основе полученных данных.
5. Заменяем lastUpdatedId на u из последнего обновления.
   
Другими словами, мы не получаем каждый раз новое состояние, а получаем только изменения состояния относительно предыдущего и изменяем ордербук соответственно.
